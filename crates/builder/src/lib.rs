...
pub fn build_game(options: &BuildOptions) -> Result<BuildResult> {
    ...
    let zip_path = options.zip_path.as_ref().map(|path| absolute_path(path)).transpose()?;
    ...
    if !manifest.contains_key("package") {
        manifest.insert("package".to_owned(), json!({"formatVersion": 3, "entry": "game.luau"}));
    }
    if manifest
        .get("package")
        ...
        != Some("game.luau")
    {
        return Err(BuildError("manifest.package.entry must be 'game.luau'".to_owned()));
    }
    if let Some(display_name) = manifest.get("displayName") {
        if display_name.as_str().is_none_or(|value| value.trim().is_empty() || value.trim().len() > 120) {
            return Err(BuildError(
                "manifest.displayName must be a non-empty string of at most 120 characters".to_owned(),
            ));
        }
    } else {
        manifest.insert("displayName".to_owned(), Value::String(game_id.clone()));
    }
    let sdk_version = manifest.get("sdkVersion").and_then(Value::as_str).map(str::to_owned);
    if let Some(sdk_version) = &sdk_version {
        validate_version(&Value::String(sdk_version.to_owned()), "manifest.sdkVersion")?;
    ...
    if authority_source.is_file() {
        ...
        package.insert("authorityEntry".to_owned(), Value::String("authority.luau".to_owned()));
    ...
    let zip_path = zip_path.map(|path| {
        if path.starts_with(&output) {
            Err(BuildError("zip path cannot be inside the package output directory".to_owned()))
        } else if path.starts_with(&source_root) {
            Err(BuildError("zip path cannot be inside the source directory".to_owned()))
        } else {
            Ok(path)
        }
    }).transpose()?;
    if let Some(zip_path) = &zip_path {
        if zip_path.is_dir() {
            return Err(BuildError(format!("zip path is a directory: {}", zip_path.display())));
        }
    ...
    let result = (|| {
        ...
        if authority_source.is_file() {
            ...
            if authority.len() > MAX_AUTHORITY_SOURCE_BYTES {
                return Err(BuildError("bundled trusted authority rules exceed the 1 MiB limit".to_owned()));
            }
            if !authority_dependencies.is_empty() {
                return Err(BuildError("trusted authority rules cannot require client Cubacadabra SDK modules".to_owned()));
            }
        ...
        copy_tree_if_present(&manifest_path.parent().unwrap().join("assets"), &staging.join("assets"))?;
        ...
        if let (Some(zip_path), Some(archive_temp)) = (&zip_path, &archive_temp) {
            fs::rename(archive_temp, zip_path).map_err(io_error("could not install package archive"))?;
        }
        Ok(())
    })();
    ...
    if result.is_err() {
        ...
        if let Some(archive_temp) = &archive_temp { let _ = fs::remove_file(archive_temp); }
    }
    result?;
    Ok(BuildResult { game_id, version, output, zip_path })
}
...
fn required_string<'a>(object: &'a Map<String, Value>, key: &str) -> Result<&'a str> {
    object.get(key).and_then(Value::as_str).ok_or_else(|| BuildError(format!("manifest.{key} must be present and a string")))
}

fn valid_game_id(value: &str) -> bool {
    (3..=64).contains(&value.len()) && value.chars().enumerate().all(|(index, c)| c.is_ascii_lowercase() || c.is_ascii_digit() || (c == '-' && index > 0 && index + 1 < value.len() && !value[..index].ends_with('-')))
}

fn validate_version(value: &Value, field: &str) -> Result<()> {
    if value.as_u64().is_some_and(|number| number >= 1) { return Ok(()); }
    let Some(value) = value.as_str() else { return Err(BuildError(format!("{field} must be SemVer or a legacy positive integer"))); };
    let core = value.split_once('+').map_or(value, |(core, _)| core);
    let core = core.split_once('-').map_or(core, |(core, _)| core);
    let parts: Vec<_> = core.split('.').collect();
    if parts.len() != 3 || parts.iter().any(|part| part.is_empty() || part.parse::<u64>().is_err()) {
        return Err(BuildError(format!("{field} must be SemVer or a legacy positive integer")));
    }
    Ok(())
}

fn resolve_effects_source(manifest: &mut Map<String, Value>, project_root: &Path) -> Result<()> {
    let Some(effects) = manifest.get("effects").cloned() else { return Ok(()); };
    let Some(object) = effects.as_object() else { return Ok(()); };
    let Some(source) = object.get("source").and_then(Value::as_str) else { return Ok(()); };
    if object.len() != 1 || source.is_empty() || source.starts_with('/') || source.contains("..") || !source.ends_with(".json") {
        return Err(BuildError("manifest.effects.source must be a relative JSON path inside the game project".to_owned()));
    }
    let path = project_root.join(source);
    let value = read_json(&path)?;
    if !value.is_object() { return Err(BuildError("manifest.effects.source must contain a JSON object".to_owned())); }
    manifest.insert("effects".to_owned(), value);
    Ok(())
}

fn validate_assets(manifest: &Map<String, Value>, project_root: &Path) -> Result<()> {
    let Some(assets) = manifest.get("assets").and_then(Value::as_object) else { return Ok(()); };
    for (kind, max, extensions, max_bytes) in [("images", 16usize, &["png", "jpg", "jpeg"][..], MAX_IMAGE_ASSET_BYTES), ("audio", 64usize, &["wav"][..], MAX_AUDIO_ASSET_BYTES)] {
        let Some(entries) = assets.get(kind) else { continue; };
        let Some(entries) = entries.as_object() else { return Err(BuildError(format!("manifest.assets.{kind} must be an object"))); };
        if entries.len() > max { return Err(BuildError(format!("manifest.assets.{kind} cannot contain more than {max} assets"))); }
        for (id, definition) in entries {
            if id.is_empty() || id.len() > 64 || !id.chars().all(|c| c.is_ascii_alphanumeric() || matches!(c, '.' | '-' | '_')) { return Err(BuildError(format!("manifest.assets.{kind} ids must use ASCII letters, numbers, dots, dashes, or underscores"))); }
            let object = definition.as_object().ok_or_else(|| BuildError(format!("manifest.assets.{kind}.{id} must be an object")))?;
            let path = object.get("path").and_then(Value::as_str).ok_or_else(|| BuildError(format!("manifest.assets.{kind}.{id}.path must be a string")))?;
            if path.starts_with('/') || path.contains("..") || !path.starts_with("assets/") || !extensions.iter().any(|extension| path.to_ascii_lowercase().ends_with(extension)) { return Err(BuildError(format!("manifest.assets.{kind}.{id}.path is invalid: {path}"))); }
            let full = project_root.join(path);
            let metadata = fs::metadata(&full).map_err(|_| BuildError(format!("manifest.assets.{kind}.{id}.path was not found: {path}")))?;
            if metadata.len() > max_bytes { return Err(BuildError(format!("manifest.assets.{kind}.{id}.path exceeds the asset size limit: {path}"))); }
        }
    }
    Ok(())
}
...
fn read_json(path: &Path) -> Result<Value> {
    ...
    serde_json::from_str(&source).map_err(|error| BuildError(format!("invalid JSON in {}: {error}", path.display())))
}
...
fn validate_output(output: &Path, source_root: &Path) -> Result<()> {
    if output.starts_with(source_root) || source_root.starts_with(output) { return Err(BuildError("output directory cannot overlap the source directory in either direction".to_owned())); }
    Ok(())
}

fn validate_existing_output(output: &Path) -> Result<()> {
    if !output.exists() { return Ok(()); }
    if !output.is_dir() { return Err(BuildError(format!("output exists and is not a directory: {}", output.display()))); }
    let marker = fs::read_to_string(output.join(BUILD_MARKER)).map_err(|_| BuildError(format!("refusing to replace existing output without a Cubacadabra build marker: {}", output.display())))?;
    if marker != BUILD_MARKER_CONTENT { return Err(BuildError(format!("refusing to replace existing output with an invalid Cubacadabra build marker: {}", output.display()))); }
    Ok(())
}
...
fn install_staging(staging: &Path, output: &Path) -> Result<()> {
    let backup = if output.exists() { Some(unique_sibling(output, "old")?) } else { None };
    if let Some(backup) = &backup { fs::remove_dir(backup).map_err(io_error("could not prepare output backup"))?; fs::rename(output, backup).map_err(io_error("could not retain previous package"))?; }
    if let Err(error) = fs::rename(staging, output) {
        if let Some(backup) = &backup { let _ = fs::rename(backup, output); }
        return Err(io_error("could not install package output")(error));
    }
    if let Some(backup) = backup { let _ = fs::remove_dir_all(backup); }
    Ok(())
}
...
fn copy_tree_if_present(source: &Path, destination: &Path) -> Result<()> {
    ...
    for entry in fs::read_dir(source).map_err(io_error("could not read asset directory"))? {
        ...
        if entry.file_type().map_err(io_error("could not inspect asset type"))?.is_dir() { copy_tree_if_present(&source_path, &destination_path)?; }
        else if entry.file_type().map_err(io_error("could not inspect asset type"))?.is_file() { fs::copy(&source_path, &destination_path).map_err(io_error("could not copy asset"))?; }
    }
    Ok(())
}
...
fn collect_files(root: &Path, current: &Path, files: &mut Vec<String>) -> Result<()> {
    for entry in fs::read_dir(current).map_err(io_error("could not enumerate package"))? {
        ...
        let path = entry.path();
        if entry.file_type().map_err(io_error("could not inspect package entry"))?.is_dir() { collect_files(root, &path, files)?; }
        else if path.file_name().and_then(|name| name.to_str()) != Some(BUILD_MARKER) { files.push(path.strip_prefix(root).unwrap().to_string_lossy().replace('\\', "/")); }
    }
    Ok(())
}
...
fn sha256_file(path: &Path) -> Result<String> {
    ...
    loop { let count = file.read(&mut buffer).map_err(io_error("could not hash package file"))?; if count == 0 { break; } digest.update(&buffer[..count]); }
    Ok(format!("{:x}", digest.finalize()))
}

fn write_zip(source: &Path, destination: &Path) -> Result<()> {
    let file = fs::File::create(destination).map_err(io_error("could not create package archive"))?;
    let mut archive = zip::ZipWriter::new(file);
    let options = zip::write::SimpleFileOptions::default().compression_method(zip::CompressionMethod::Deflated);
    for name in files_under(source)? {
        archive.start_file(&name, options).map_err(|error| BuildError(format!("could not write package archive: {error}")))?;
        let bytes = fs::read(source.join(&name)).map_err(io_error("could not read package file"))?;
        archive.write_all(&bytes).map_err(io_error("could not write package archive"))?;
    }
    archive.finish().map_err(|error| BuildError(format!("could not finish package archive: {error}")))?;
    Ok(())
}
...
fn bundle_modules(entry: &Path, source_root: &Path, dependencies: &mut BTreeMap<String, Value>) -> Result<String> {
    ...
    for (module_id, module) in &modules {
        ...
        for (specifier, dependency) in &module.routes { output.push(format!("    [{}] = {},", json_string(specifier), json_string(dependency))); }
        output.push("}".to_owned());
        output.push(format!("__modules[{}] = function(require)", json_string(module_id)));
...
fn visit(module_id: String, source: String, path: Option<&Path>, source_root: &Path, modules: &mut BTreeMap<String, Module>, stack: &mut Vec<String>, dependencies: &mut BTreeMap<String, Value>) -> Result<String> {
    ...
    for specifier in require_specifiers(&source, &module_id)? {
        let dependency_id = if specifier.starts_with("@cubacadabra/") {
            let dependency_source = sdk_source(&specifier).ok_or_else(|| BuildError(format!("unknown Cubacadabra SDK module: {specifier}")))?;
            dependencies.insert(specifier.clone(), json!({"source": "cubacadabra-preview-sdk", "sha256": sha256_bytes(dependency_source.as_bytes())}));
            visit(specifier.clone(), dependency_source.to_owned(), None, source_root, modules, stack, dependencies)?
        } else {
            let requiring_path = path.ok_or_else(|| BuildError(format!("{module_id}: SDK modules cannot require game source modules")))?;
            let dependency = resolve_local_module(&specifier, requiring_path, source_root, &module_id)?;
...
fn resolve_local_module(specifier: &str, requiring_path: &Path, source_root: &Path, module_id: &str) -> Result<PathBuf> {
    if !(specifier.starts_with("./") || specifier.starts_with("../")) || specifier.contains('\\') { return Err(BuildError(format!("{module_id}: require path must start with './', '../', or '@cubacadabra/' and use forward slashes"))); }
    let base = requiring_path.parent().unwrap_or(source_root);
    let mut clean = PathBuf::new();
    for component in Path::new(specifier).components() { match component { std::path::Component::CurDir => {}, std::path::Component::ParentDir => { if !clean.pop() { return Err(BuildError(format!("{module_id}: require must stay inside src/"))); } }, std::path::Component::Normal(value) => clean.push(value), _ => return Err(BuildError(format!("{module_id}: invalid require path"))), } }
    let unresolved = base.join(clean);
    let candidates = if unresolved.extension().is_some() { vec![unresolved.clone()] } else { vec![unresolved.with_extension("luau"), unresolved.with_extension("lua"), unresolved.join("init.luau"), unresolved.join("init.lua")] };
    let matches: Vec<_> = candidates.into_iter().filter(|candidate| candidate.is_file()).collect();
    match matches.as_slice() { [] => Err(BuildError(format!("{module_id}: required module not found: {specifier}"))), [match_path] => Ok(match_path.clone()), _ => Err(BuildError(format!("{module_id}: required module is ambiguous: {specifier}"))) }
}
...
fn require_specifiers(source: &str, module_id: &str) -> Result<Vec<String>> {
    ...
    while cursor < bytes.len() {
        ...
        if character.is_ascii_alphabetic() || character == '_' {
            ...
            if argument >= bytes.len() || !matches!(bytes[argument] as char, '\'' | '"') { return Err(BuildError(format!("{module_id}: require paths must be static quoted strings"))); }
            let quote = bytes[argument] as char;
            let mut end = argument + 1;
            while end < bytes.len() && bytes[end] as char != quote { if bytes[end] == b'\\' { return Err(BuildError(format!("{module_id}: require paths cannot contain escapes"))); } end += 1; }
            if end >= bytes.len() { return Err(BuildError(format!("{module_id}: unterminated require path"))); }
...
fn skip_space_comments(source: &str, mut cursor: usize) -> usize { while cursor < source.len() { if source.as_bytes()[cursor].is_ascii_whitespace() { cursor += 1; } else if source[cursor..].starts_with("--") { cursor = skip_comment(source, cursor); } else { break; } } cursor }
fn skip_quoted(source: &str, mut cursor: usize, quote: char) -> usize { cursor += 1; while cursor < source.len() { if source.as_bytes()[cursor] == b'\\' { cursor = (cursor + 2).min(source.len()); } else if source.as_bytes()[cursor] as char == quote { return cursor + 1; } else { cursor += 1; } } source.len() }
fn skip_comment(source: &str, mut cursor: usize) -> usize { cursor += 2; while cursor < source.len() && source.as_bytes()[cursor] != b'\n' { cursor += 1; } cursor }
fn json_string(value: &str) -> String { serde_json::to_string(value).unwrap_or_else(|_| "\"\"".to_owned()) }
fn sha256_bytes(bytes: &[u8]) -> String { let mut digest = Sha256::new(); digest.update(bytes); format!("{:x}", digest.finalize()) }
