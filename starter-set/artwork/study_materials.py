...
def material(name, color, kind='plain', tint=False, display=None):
    ...
    noise=n.new('ShaderNodeTexNoise'); noise.inputs['Scale'].default_value=110 if kind=='denim' else 175
    ...
    strength={'plain':.025,'fleece':.19,'denim':.70,'hair':.15,'rib':.10}.get(kind,.06)
    ...
    if kind in ('denim','rib'):
        ...
        mix=n.new('ShaderNodeMixRGB'); mix.blend_type='MULTIPLY'; mix.inputs[0].default_value=.22 if kind=='denim' else .12
...
def materials():
    ...
    return {
        ...
        'denim':material('Indigo crosswoven denim',(.048,.080,.13),'denim'),
        'denim-hem':material('Turned indigo denim',(.065,.105,.16),'denim'),
        'thread':material('Denim topstitch',(.24,.23,.19)),
        ...
        'hair':material('Chestnut swept hair',(.12,.044,.018),'hair'),
    }
